class Snake {
  ...
  constructor(initiallength, soundManager) {
    this.currentCells = [];
    ...
    for(let i=0; i<initiallength; i++) {
      this.colorCell(i+1); 
      this.currentCells.push(i+1);
    }
    this.spawnWorm();
  }

  // Method for coloring a specific cell
  colorCell(n) { 
    $(".grid__cell:nth-child("+n+")").css('background', '#20C20E');
  }

  // Method for uncoloring a specific cell but ignore if its worm
  uncolorCell(n) {
    if(n !== this.wormPosition) {
      $(".grid__cell:nth-child("+n+")").css('background', 'black');
    }
  }

  // Method for rendering the snake
  renderSnake() {
    for(let i =0; i<400; i++) {
  ...
  // Method for randomly spawning a worm and it won't be on snake body
  spawnWorm() {
    let rand = 1;
  ...
  // Method for following the snake head path
  shiftBody(head) {
    let cells = this.currentCells;
    cells.push(head);
    this.currentCells = cells.slice(1);
    this.renderSnake();
  }

  // Method for moving to different direction based on user action
  move(direction) {
    this.soundManager.playAudioIfUnmuted('beep');
    // Ccancelling existing moves
    clearInterval(this.moveInstance);
    // Creating a new move instance
    this.moveInstance = setInterval(() => {
      let ignored = false;
      let cells = this.currentCells;
      const Head = cells[cells.length-1];
      const headPos = cells.indexOf(Head);
      // TODO: When snake tries to go through itself, kill it
      // if(headPos > -1 && headPos !== cells.length-1) {
      ...
      // When the head touches the worm, eat it and grow the snake
      // Also re-render the snake, increase score, play eating audio
      if(this.wormPosition === this.currentCells[this.currentCells.length-1]) {
        this.currentCells.push(this.wormPosition);
        this.spawnWorm();
        this.renderSnake();
        this.score = this.score + 1;
        $(".score").text(this.score);
        this.soundManager.playAudioIfUnmuted('eat');
      }
    }, 200);
  }
...
function initGrid() {
  for(let i =0; i<400; i++) {
    $(".grid__container").append('<div class="grid__cell"></div>');
  }
}
...
class SoundManager {
  ...
  renderSoundIcon(status) {
    if(!status) {
      $(".sound__icon").addClass("fa-volume-mute");
      $(".sound__icon").removeClass("fa-volume-up");
    } else {
      $(".sound__icon").addClass("fa-volume-up");
      $(".sound__icon").removeClass("fa-volume-mute");
    }
  }

  toggleMuteIcon() {
    const status = localStorage.getItem('mute');
    if(!status) {
      localStorage.setItem('mute', 'true');
    } else {
      localStorage.removeItem('mute');
    }
    this.renderSoundIcon(localStorage.getItem('mute'));
  }
}
...
function main() {
  ...
  // Render sound icon based on mute status
  soundManager.renderSoundIcon(localStorage.getItem('mute'));
  // Initialize the grid
  initGrid();
  // Create a Snake instance
  let snake = new Snake(5, soundManager);

  // All the soft key bind methods
  $(".up__btn").click(() => {
    snake.move('up');
  });
  ...
  // All the hardware key bind methods
